...
$sql = "SELECT id_k, Tytul, Autor FROM ksiazki";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr> <td>" . $row["id_k"]. "</td><td> " . $row["Tytul"]. " </td><td> " . $row["Autor"]. "</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 results";
...
if (isset($_POST['usun'])) {
    ...
    $select = mysqli_query($conn, "SELECT id_k FROM ksiazki WHERE id_k = '".$_POST['id_k']."' ") or exit(mysqli_error($conn));
    ...
    if (count($errors) == 0) {
        ...
        $query = "DELETE FROM ksiazki WHERE id_k = ".$_POST['id_k']." ";
        mysqli_query($conn, $query);
        array_push($errors, "Książka została usunięta");
...
<tr><td colspan="2">Usuwanie książki</td></tr>
<tr><td style="width:60%;">Podaj ID książki:</td><td><input style="background-color: rgba(20, 52, 157, 0.8);width:40%;color: white;"min="0" type="number" name="id_k"></td></tr>
<tr><td colspan="2"><input type="submit" class="myButton" style="width:30%" value="Usuń" name = "usun"></td></tr>
