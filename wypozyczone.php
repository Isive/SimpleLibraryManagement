...
$sql = "SELECT k.id_k,k.Tytul,k.Autor,k.Rok_wydania,w.data_wypozyczenia FROM ksiazki k JOIN wypozyczenia w ON k.id_k = w.id_ksiazki WHERE w.id_uzytkownika = '".$_SESSION['id']."' ";
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<th>ID</th><th>Tytuł</th><th>Autor</th><th>Rok wydania</th><th>Data wypożyczenia</th>";
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr> <td>" . $row["id_k"]. "</td><td> " . $row["Tytul"]. " </td><td> " . $row["Autor"]. "</td><td>  " . $row["Rok_wydania"]. "</td><td>  " . $row["data_wypozyczenia"]. "</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 results";
